...
describe('Contact component', () => {
  ...
  it('renders the resume anchor text', () => {
    const resumeLink = screen.getByRole('link', { name: /click on this link to find my updated resume/i });
    expect(resumeLink).toBeInTheDocument();
  });

  it('has a resume link pointing to the PDF path', () => {
    const resumeLink = screen.getByRole('link', { name: /click on this link to find my updated resume/i });
    expect(resumeLink).toHaveAttribute('href', 'assets/Kyle-McColgan-Resume-Spectrum-2025.pdf');
  });
